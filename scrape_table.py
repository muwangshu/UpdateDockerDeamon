...
def main():
    try:
        ...
        # 保存为JSON文件，使用UTF-8编码
        output_file = 'updatedocker.sh'
        with open(output_file, 'w+', encoding='utf-8') as f:
            f.write("sudo mkdir -p /etc/docker\n")
            f.write("# 写入配置文件\n")
            f.write("sudo tee /etc/docker/daemon.json <<-'EOF'\n")
            json.dump(data, f, ensure_ascii=False, indent=2)
            f.write("\nEOF\n")
            f.write("# 重启docker服务\n")
            f.write("sudo systemctl daemon-reload && sudo systemctl restart docker\n")
